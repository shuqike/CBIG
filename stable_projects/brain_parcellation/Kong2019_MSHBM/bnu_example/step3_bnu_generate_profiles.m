function CBIG_MSHBM_generate_profiles(seed_mesh,targ_mesh,out_dir,sub,sess,split_flag)
    ...
    %   For data in fsaverage space:
    %   <out_dir>/profiles/sub<sub_id>/sess<sess_id>/lh.sub<sess_id>_sess<sess_id>_<targ_mesh>_roi<seed_mesh>_surf2surf_profile.nii.gz
    %   <out_dir>/profiles/sub<sub_id>/sess<sess_id>/lh.sub<sess_id>_sess<sess_id>_<targ_mesh>_roi<seed_mesh>_surf2surf_profile.nii.gz
    %   For data in fs_LR_32k space:
    ...
    if(strcmp(targ_mesh,'fs_LR_32k'))
        ...
        CBIG_shuqi_ComputeCorrelationProfile(seed_mesh,targ_mesh, profile_file, 'NONE', '0.1', fMRI_list, 'NONE', censor_list, split_flag);
    
    elseif(~isempty(strfind(targ_mesh,'fsaverage'))) 
        if(split_flag~='0')
            lh_profile_file = [fullfile(out_profile_dir,['lh_sub' sub '_sess' sess '_' targ_mesh '_roi' seed_mesh '_surf2surf_profile1.nii']) fullfile(out_profile_dir,['lh_sub' sub '_sess' sess '_' targ_mesh '_roi' seed_mesh '_surf2surf_profile2.nii'])];
            rh_profile_file = [fullfile(out_profile_dir,['rh_sub' sub '_sess' sess '_' targ_mesh '_roi' seed_mesh '_surf2surf_profile1.nii']) fullfile(out_profile_dir,['rh_sub' sub '_sess' sess '_' targ_mesh '_roi' seed_mesh '_surf2surf_profile2.nii'])];
        else
        ...
        bnu_compute_corr(seed_mesh,targ_mesh, lh_profile_file, rh_profile_file, '0.1', lh_fMRI_list, rh_fMRI_list, censor_list, split_flag);
    else
