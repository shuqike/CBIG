function CBIG_MSHBM_avg_profiles(seed_mesh,targ_mesh,out_dir,sub_list,start_sess,end_sess)

    % CBIG_MSHBM_avg_profiles(seed_mesh,targ_mesh,out_dir,sub_list,start_sess,end_sess)
    ...
    %   - sub_list: (array)
    %
    %     The list of subjects, e.g. '{'sub-BSC23', 'sub-BSC40'}'.
    %
    %   - start_sess, end_sess: (string)
    %
    %     The range of sessions, e.g. '1', '2'.
    %
    % Output:
    %
    ...
    for sub = sub_list
        for sess = str2num(start_sess):str2num(end_sess)
            sub = char(sub);
            sess = num2str(sess);
