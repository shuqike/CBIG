...
for i = 1:length(varargin1)
    ...
    if(~isempty(strfind(seed_mesh, 'fsaverage')))
        ...
        [~, rh_seed_series, ~] = read_fmri(varargin2{i});
...
if(~isempty(strfind(target, 'fsaverage'))) 
    ...
    for i = 1:length(varargin2)
        ...
        [~, lh_seed_series, ~] = read_fmri(varargin1{i});
...
if(split_data~=0 && length(varargin1)==1)
    for run = 1:split_data
        ...
        if(str2num(threshold) < 1)
            ...
            write_fmri(fullfile(output_file1(run)), input_series, corr_mat1(:,:,run)', [input_size(1:end-1) size(corr_mat1, 1)]);
            ...
            if(~isempty(strfind(target, 'fsaverage')))
                ...
                write_fmri(output_file2(run), input_series, corr_mat2(:,:,run)', [input_size(1:end-1) size(corr_mat2, 1)]);
            end
        end
    end
else
